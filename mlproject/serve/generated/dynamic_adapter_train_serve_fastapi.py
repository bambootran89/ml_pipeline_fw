# Auto-generated FastAPI serve for dynamic_adapter_train_serve
...
app = FastAPI(
    title="dynamic_adapter_train_serve API",
    version="1.0.0",
...
class HealthResponse(BaseModel):
    ...
    features: List[str] = ['HUFL', 'MUFL', 'mobility_inflow']
...
class ServeService:
    ...
    def __init__(self, config_path: str) -> None:
        ...
        if self.mlflow_manager.enabled:
            ...
            print(
                f"[ModelService] Loading model: fitted_kmeans_dynamic from "
                f"kmeans_dynamic (alias: latest)..."
            )
            component = self.mlflow_manager.load_component(
                name=f"{experiment_name}_kmeans_dynamic",
                alias="latest",
            )
            if component is not None:
                self.models["fitted_kmeans_dynamic"] = component

            print(
                f"[ModelService] Loading model: fitted_xgboost_model from "
                f"xgboost_model (alias: latest)..."
            )
            component = self.mlflow_manager.load_component(
            ...
            if component is not None:
                self.models["fitted_xgboost_model"] = component
    ...
    def predict_tabular_batch(
        ...
        model = self.models.get("fitted_kmeans_dynamic")
        if model is not None:

            base = context.get("imputed_data")
            ...
            if x is not None:
                ...
                p = model.predict(inp)
                results["kmeans_dynamic_predictions"] = p.tolist()
                context["kmeans_dynamic_predictions"] = p
                if return_probabilities and hasattr(model, "predict_proba"):
                    try:
                        pb = model.predict_proba(inp)
                        results["kmeans_dynamic_predictions_probabilities"] = (
                            pb.tolist()
                        )
                    except Exception:
                        pass

        model = self.models.get("fitted_xgboost_model")
        if model is not None:
            ...
            if x is not None:
                ...
                metadata["n_samples"] = len(inp)
                p = model.predict(inp)
                results["xgboost_model_predictions"] = p.tolist()
                context["xgboost_model_predictions"] = p
                if return_probabilities and hasattr(model, "predict_proba"):
                    try:
                        pb = model.predict_proba(inp)
                        results["xgboost_model_predictions_probabilities"] = (
                            pb.tolist()
                        )
                    except Exception:
                        pass
    ...
    def predict_timeseries_multistep(
        ...
        if model is not None:
            ...
            if feat is not None:
                ...
                results["kmeans_dynamic_predictions"] = out.tolist()
                context["kmeans_dynamic_predictions"] = out
        ...
        if model is not None:
            ...
            if feat is not None:
                ...
                if preds.ndim == 1:
                    ...
                    out = np.concatenate(preds, axis=0)
                results["xgboost_model_predictions"] = out.tolist()
                context["xgboost_model_predictions"] = out
