...
class ModelService:
    ...
    def _load_models(self) -> None:
        if not self.mlflow_manager.enabled:
            return
        experiment_name = self.cfg.experiment.get("name", "standard_train_serve")
        print(f"[ModelService] Loading model: fitted_train_model "
              f"(alias: latest)...")
        component = self.mlflow_manager.load_component(
    ...
    def predict_tabular_batch(
        ...
        if model is not None:
            ...
            if features is not None:
                ...
                results["train_model_predictions"] = preds.tolist()
                if return_probabilities and hasattr(model, "predict_proba"):
                    try:
                        proba = model.predict_proba(x_input)
                        key_prob = "train_model_predictions_probabilities"
    ...
    def predict_timeseries_multistep(
        ...
        if model is not None:
            ...
            if features is not None:
                ...
                results["train_model_predictions"] = preds_2d.tolist()
        return {"predictions": results, "metadata": metadata}
